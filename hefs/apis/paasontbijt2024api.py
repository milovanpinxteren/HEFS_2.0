...
class Paasontbijt2024API:
    ...
    def get_shopify_orders(self):
        shopify_access_token = settings.PAASONTBIJT_ACCESS_TOKEN
        headers = {"Accept": "application/json", "Content-Type": "application/json",
                   "X-Shopify-Access-Token": shopify_access_token}
        response = requests.get(url="https://b88885-3.myshopify.com/admin/api/2024-01/orders.json", headers=headers)
    ...
    def handle_shopify_orders(self, orders):
        ...
        for order in orders:
            if str(order['name'])[0] == '0':
                order['name'] = '1' + order['name'][1:]
    ...
    def add_to_new_orders(self, order):
        ...
        for newOrderLine in order['line_items']:
            if newOrderLine['fulfillable_quantity'] > 0:
                ...
                NewOrders.objects.create(conversieID=int(order['name']),
                                         shopifyID=order['id'],
